...
pacman::p_load(readr, readxl, tidyr, dplyr, lubridate, leaflet, caret, ggplot2, ggmosaic, geonames, shiny, shinydashboard, DT)
```
...
```{r}
ggplot(df_saltolinea, aes(x=REGVI))+geom_bar(aes(fill=REGVI))+theme_minimal() +theme(axis.text.x = element_text(angle = 0, vjust = 0.95, hjust=1))
...
df_saltolinea <- df_filtrado %>%
  mutate(TAMANO = gsub("(habitantes)", "hab", TAMANO))
