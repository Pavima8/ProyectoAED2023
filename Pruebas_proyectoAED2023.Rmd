...
```{r}
df <- full_join(persona,hogar, by="ID_VIV")
df_num <- full_join(persona_2,hogar_2, by = "ID_VIV")

df <- df %>% 
  select(-PERIODO.y,-TAMANO.y,-IDQ_PV.y,-CA.y,-FACCAL.y) %>% 
...
columnas_int_factor <- c("TAMANO","REGVI","TIPOVIV","NAC_HO","SEXO","EC","NACIM","COCINA","NAC","NACNACIMESP","NACIMPADRE","NACIMMADRE","OCUPA","PAREJA","SEXOPAR","NACPAR","ECPAR","HIJOSDEAMBOS","ESTUDIOS","F_CONS_EDI","HABVI","METROSVI","ASEOS","DORMITORIOS","COMEDORES","TRASTEROS","OTRASHAB")
...
df_merged_1 <- df_merged %>% 
  select(c(TAMANO,PROVINCIA,CA,ASEOS,COMEDORES,TRASTEROS,OTRASHAB,HABVI,METROSVI,TIPOVIV,F_CONS_EDI,REGVI.1,REGVI.2,REGVI.3,REGVI.4,NPV,COCINA.1,COCINA.6)) %>% 
  filter(NPV == 1)

df_merged_2 <- df_merged %>% 
  select(c(TAMANO,PROVINCIA,CA,ASEOS,ESTUDIOS,COMEDORES,TRASTEROS,OTRASHAB,HABVI,METROSVI,TIPOVIV,F_CONS_EDI,REGVI.1,REGVI.2,REGVI.3,REGVI.4,NPV,COCINA.1,COCINA.6,)) %>% 
  filter(NPV == 1)

df_merged_3 <- df_merged %>% 
  select(c(EC.1,EC.2,EC.3,EC.4,EC.5,METROSVI,)) %>% 
  filter(NPV == 1)
...
deteccion_outliers(df_por_familia$DENSIDADVI)
