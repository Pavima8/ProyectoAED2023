...
df_saltolinea <- df_saltolinea %>%
  mutate(
    ESTUDIOS = gsub("Llegó al último curso de ESO, EGB o Bachiller Elemental o tiene el Certificado de Escolaridad o de Estudios Primarios", "Acaba ESO", ESTUDIOS),
    ESTUDIOS = gsub("Diplomatura universitaria, Arquitectura Técnica, Ingeniería Técnica o equivalente", "Diplom univ", ESTUDIOS),
    ESTUDIOS = gsub("Fue a la escuela 5 años o más pero no llegó al último curso de la ESO, EGB o Bachiller Elemental", "Escuela 5 años", ESTUDIOS),
    ESTUDIOS = gsub("FP grado medio, FP I, Oficialía Industrial o equivalente, Grado Medio de Música y Danza, Certificados de Escuelas Oficiales de Idiomas", "FP grado med", ESTUDIOS),
    ESTUDIOS = gsub("Bachiller \\(LOE, LOGSE\\), BUP, Bachiller Superior, COU, PREU", "Bachiller", ESTUDIOS),
    ESTUDIOS = gsub("Master oficial universitario \\(a partir de 2006\\), Especialidades Médicas o análogos", "Master univ", ESTUDIOS),
    ESTUDIOS = gsub("FP grado superior, FP II, Maestría industrial o equivalente", "FP grado sup", ESTUDIOS),
    ESTUDIOS = gsub("Sabe leer y escribir pero fue menos de 5 años a la escuela", "Si leer&escribir", ESTUDIOS),
    ESTUDIOS = gsub("Licenciatura, Arquitectura, Ingeniería o equivalente", "Licenciatura", ESTUDIOS),
    ESTUDIOS = gsub("Grado Universitario o equivalente" , "Grado Univ", ESTUDIOS ),
    ESTUDIOS = gsub("No sabe leer o escribir" , "No leer/escribir", ESTUDIOS )
  )%>%mutate(
...
*Para no abusar de los diagramas de barras se ha representado con un mosaico, aunque parece que queda más claro con barras*

Hemos recogido en diferentes diagramas de barras cómo se distribuyen las variables respecto a lugar de nacimiento:

```{r}
plot_grid(
  ggplot(df_saltolinea, aes(x=ESTUDIOS))+geom_bar(aes(fill=NACIM), position = "dodge")+theme_minimal() +theme(axis.text.x = element_text(angle = 80, vjust = 0.95, hjust=1))+labs(title = "Diagrama estudios",
       x = NULL,
  ...
  ggplot(df_saltolinea, aes(x=NHIJO))+geom_bar(aes(fill=NACIM), position = "dodge")+theme_minimal() +theme(axis.text.x = element_text(angle = 80, vjust = 0.95, hjust=1))+labs(title = "Diagrama número de hijos",
       x = NULL,
       y = NULL, fill = "Donde nacieron"),
  ncol = 2, nrow = 2
)
